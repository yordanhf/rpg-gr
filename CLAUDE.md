...
- Solución `RpgGr.sln` con dos proyectos (**net9.0** — SDK disponible es .NET 9, no .NET 8):
  ...
  - `src/ConsoleProto`: prototipo de consola, ahora un **bucle de comandos** real (`kill <rat|deer|beast>`, `shape [target]`, `simulate [rounds]`, `reset`, `flee`/`stop`, `listk`, `quit`), no una pelea guionizada. `CharacterLoader` deserializa cada personaje directamente en un `Combatant` vía `System.Text.Json` (sus propiedades `required` obligan a que el JSON tenga todas las stats).
...
- **`simulate [rounds]`** (por defecto 1000) es una herramienta de **diagnóstico**, en `src/ConsoleProto/Simulation.cs`: por cada NPC (rat/deer/beast) resuelve N rondas de un ataque por lado con `AttackResolver.Resolve` directamente (sin `CombatEngine`, sin delays), **ignorando el HP** (nadie muere). Imprime, por Player/NPC: cantidad de golpes por tipo (Miss, Graze… Critical), hit rate, daño medio por golpe y por ronda. Hallazgo con los valores actuales: casi todo es `Graze` (techo de daño del player = 40/300×30 = 4), sin `Hit`+ salvo algún crítico; hacen falta arma/skills para niveles altos.
