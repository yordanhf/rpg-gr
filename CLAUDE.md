...
- **Tiendas (DECIDIDO parcialmente e implementado el lado de compra):** el jugador compra al 100% del precio base (`Weapon.Value`/`Armor.Value`/`Backpack.Value`, en gold). **Falta implementar**: vender a la tienda al 25% del precio base, el **tope de venta** (nada se vende por más de X gold, valor exacto pendiente), y el botín (algunos NPC sueltan gold al morir; los animales — rat/deer/beast — sueltan un material como piel/colmillo vendible en la tienda correspondiente). Sin botín todavía, el jugador **no tiene forma de ganar gold** en el juego — arranca en 0.
  - `World.Shop` (referenciado desde `Room.Shop`, nullable): listas de `Weapon`/`Armor` + un `Backpack` opcional a la venta. Comprar siempre da una **copia nueva** (el stock no se agota). Comandos `list` (ver stock y precios) y `buy <item>`.
