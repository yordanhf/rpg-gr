...
- **`CharacterSave`** (`src/World/CharacterSave.cs`) es un DTO plano, separado de `Combatant` (que lleva estado de ejecución: cooldowns, buffs, estado de combate). Guarda nombre, **raza por id**, **habitación por id (`roomId`)**, HP/MP/gold actuales y máximos, las 9 stats/skills, arma empuñada + `UnarmedWeapon` + armas envainadas, armadura puesta + en mano, mochila + contenido. Lleva `Version` para poder migrar el formato. `CharacterSave.From(combatant, roomId)` y `ToCombatant(findRace)` hacen la conversión. **No se guardan** cooldowns, buffs ni los emotes personalizados del arma.
  - **Bug corregido:** si el personaje guarda con un arma real empuñada (no las manos vacías), `Combatant.Weapon` nunca capturaba su respaldo de "manos vacías" (`_unarmedWeapon`) al cargar, dejando `sheath` roto para siempre en esa sesión. Ahora `CharacterSave` guarda `UnarmedWeapon` por separado y `ToCombatant` primero asigna ese respaldo (para que se capture) y luego el arma real. Saves viejos sin el campo reciben un "Fists" sintético de respaldo.
