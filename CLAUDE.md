...
- **Tiendas (DECIDIDO e implementado, salvo el tope de venta):** el jugador compra al 100% del precio base (`Weapon.Value`/`Armor.Value`/`Backpack.Value`, en gold) y **vende** a la tienda al **25%** (`CombatConstants.ShopSellFraction`), redondeado, con mínimo 1 gold (para que ni un item de valor 1 se venda por 0). **Falta**: el tope de venta (nada se vende por más de X gold, valor exacto pendiente).
