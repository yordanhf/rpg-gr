...
- **Latones de basura (DECIDIDO e implementado):** `World.TrashCan` (mismo patrón que `Shop`/`Healer`/`Trainer`/`Blacksmith`), sin capacidad ni límite. Por ahora **2**: el `scrap bin` en Millford Trading Post y el `slag heap` en Millford Forge. Comando `drop <item> into trash` — busca el item en los mismos 4 sitios que `sell` (armas envainadas, armadura en mano, mochila, items sueltos en mano); si sigue puesto/empuñado, pide quitárselo/envainarlo primero, igual que `sell`. **Pide confirmación explícita** (`Type yes to confirm`, lectura directa por `Console.ReadLine`, igual que el prompt de nombre en `create`/`continue`) porque **destruye el item para siempre** — cualquier respuesta que no sea exactamente "yes" cancela sin tocar nada.
...
- Solución `RpgGr.sln` con tres proyectos (**net9.0** — SDK disponible es .NET 9, no .NET 8):
  ...
  - `src/ConsoleProto`: prototipo de consola, ahora un **bucle de comandos** real (`create`, `continue`, `look [target]`, movimiento `n/s/e/w/u/d`, `kill <target>`, `shape [target]`, `bandage [target]`, `flee`/`stop`, `wield`/`sheath`/`wear`/`remove`, `hands`, `i`/`inventory`, `gold`, `xp`, `score`, `take`/`get`/`loot`, `list`/`buy`/`sell`, `heal`, `train`/`levelup`, `repair`/`condition`, `drop <item> into trash`, `simulate [rounds] [npc]`, `reset`, `listk`, `quit`), no una pelea guionizada. `CharacterLoader` deserializa cada personaje directamente en un `Combatant` vía `System.Text.Json` (sus propiedades `required` obligan a que el JSON tenga todas las stats).
  - **Alias cortos de comandos (DECIDIDO e implementado):** igual que `look`→`l` e `inventory`→`i` ya existían, la mayoría de los comandos frecuentes tienen ahora una forma corta — `kill`→`k`, `bandage`→`bd`, `flee`/`stop`→`f`, `gold`→`g`, `xp`→`x`, `heal`→`h`, `score`→`sc`, `train`→`tr`, `levelup`→`lu`, `repair`→`rep`, `condition`→`c`, `wield`→`wi`, `sheath`→`sh`, `wear`→`wr`, `remove`→`rm`, `hands`→`ha`, `take`/`get`/`loot`→`t`, `list`→`ls`, `buy`→`b`, `sell`→`se`, `quit`/`exit`→`q`. Se dejaron **sin alias** los de un solo caracter que chocarían con movimiento (`n/s/e/w/u/d` siempre tienen prioridad) y los poco frecuentes/sensibles (`create`, `continue`, `reset`, `shape`, `simulate`, `listk`, `drop`). Letras elegidas por el desarrollador de IA sin pedir confirmación una por una — avisar si alguna choca con la intuición del desarrollador y se reasigna.
  - **Confirmación de `drop ... into trash` acepta `y` además de `yes`** (antes solo `yes`).
  - **Alias personalizados del jugador (DECIDIDO e implementado):** comando `alias <nombre> <comando...>` define/sobreescribe un alias propio (ej. `alias ee eat pie from pack`); `alias` solo lista todos los tuyos; `alias <nombre>` muestra a qué expande ese en concreto; `unalias <nombre>` lo borra. **No puede pisar ningún comando o alias predefinido del juego** (incluyendo movimiento: `n/s/e/w/u/d`/`north`/etc.) — se valida contra `reservedCommandWords` en `Program.cs`, y si choca se rechaza con un mensaje explícito, sin tocar nada. Al escribir el nombre del alias, se expande **una sola vez** (sin alias-de-alias) a su texto guardado, y cualquier palabra que escribas después del nombre se añade al final de la expansión — así `alias ka kill` deja que `ka rat` corra como `kill rat`. Se persiste por personaje en `CharacterSave.Aliases` (`Combatant.Aliases`, diccionario case-insensitive), con el mismo patrón de compatibilidad hacia atrás que `ProfessionId` (default vacío, así un save viejo sin esa clave no revienta `continue`).
