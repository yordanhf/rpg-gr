...
- **Bulk e inventario (DECIDIDO e implementado):** `Bulk` es **fraccionario** (`double`): 2 = arma a dos manos o pieza grande, 1 = normal, 1/2 o 1/4 = cosas pequeñas (pieles, colmillos…).
  - **Manos:** pool de 2 unidades de bulk (`CombatConstants.HandCapacity`). Solo se puede empuñar **un arma a la vez** (no hay dual-wield en el combate todavía, aunque el modelo ya lo permitiría a futuro).
  - **Funda:** toda arma real (no las naturales tipo Fists/Bite) tiene su funda implícita, **sin costo de bulk**. `Weapon.IsUnarmed` marca las armas "naturales" (Fists/Bite/Antlers/Claws) — no tienen funda, no se pueden desequipar. `Combatant.SheathedWeapons` guarda las armas propias no empuñadas. Comandos: `wield <arma>` (empuña, envaina lo que llevaras antes), `sheath [arma]` (envaina lo empuñado), `hands` (qué llevas en las manos). `Combatant.SheathCurrentWeapon()` es el gancho que usará el futuro `buy`/loot/pickup para envainar automáticamente cuando el jugador reciba algo nuevo (pendiente conectarlo, todavía no hay tienda/botín).
  - **Mochila:** el personaje **nace sin mochila** — se compra en la tienda (pendiente). Tiene su propia `Capacity` de bulk (mínimo 2 para la inicial; puede haber mochilas más grandes después). Se lleva puesta, sin costo extra, y **solo se puede tener una**. `Combatant.Backpack`/`BackpackItems`, `TryStoreInBackpack`/`UsedBackpackBulk`/`FreeBackpackBulk`.
  - **5 slots de armadura** por parte del cuerpo (`BodySlot`) — **Head** (helm), **Torso** (armour/hauberk), **Arms** (vambraces/gauntlets), **Legs** (steel leggings/mail pants), **Feet** (pelt boots, etc.) — y una pieza puede ocupar **más de un slot** (ej. unos "mail pants" que incluyen los zapatos = Legs+Feet a la vez). `Armor.Slots` (`HashSet<BodySlot>`, obligatorio) + `Armor.Bulk` (cuánto ocupa en mano/mochila mientras no está puesta). `Combatant.TryEquipArmor` rechaza si algún slot ya está cubierto por otra pieza puesta; `UnequipArmor` la quita.
  - Comando `i`/`inventory`: resumen de manos, armas envainadas, armadura puesta, mochila (con bulk usado/total) y gold.
  - Todo esto se persiste en `CharacterSave` (`SheathedWeapons`, `Backpack`, `BackpackItems`, más `Bulk`/`Slots`/`IsUnarmed` en las fichas de arma/armadura).
