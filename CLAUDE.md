...
- **Pendiente (persistencia, siguiente paso):** guardar/cargar el personaje (`CharacterSave` separado de `Combatant`, `ICharacterStore` con implementación JSON en `saves/<nombre>.json`, luego EF Core), guardando raza por id y `roomId`. Después: más habitaciones, NPCs que se mueven por áreas restringidas (`roam`), puertas, etc.
