...
- Solución `RpgGr.sln` con dos proyectos (**net9.0** — SDK disponible es .NET 9, no .NET 8):
  ...
  - `src/ConsoleProto`: prototipo de consola, ahora un **bucle de comandos** real (`kill <rat|deer|beast>`, `shape [target]`, `reset`, `flee`/`stop`, `listk`, `quit`), no una pelea guionizada. `CharacterLoader` deserializa cada personaje directamente en un `Combatant` vía `System.Text.Json` (sus propiedades `required` obligan a que el JSON tenga todas las stats).
