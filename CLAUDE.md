...
- Solución `RpgGr.sln` con dos proyectos (**net9.0** — SDK disponible es .NET 9, no .NET 8):
  ...
  - `src/ConsoleProto`: prototipo de consola, ahora un **bucle de comandos** real (`kill <rat|deer|beast>`, `reset`, `flee`/`stop`, `quit`), no una pelea guionizada. `CharacterLoader` deserializa cada personaje directamente en un `Combatant` vía `System.Text.Json` (sus propiedades `required` obligan a que el JSON tenga todas las stats).
- Persistencia en **JSON** (no BD todavía): `src/ConsoleProto/data/player.json` y `data/npcs/{rat,deer,beast}.json`, copiados a la salida de compilación. Personajes de nivel 1:
  ...
  - Rat/Deer/Beast: 5/15/25 parejo en las 8 stats+skills, 25/45/60 HP, con un "arma natural" (Bite/Antlers/Claws) cuyos stats de combate están todos en 0 — solo aporta el nombre para emotes futuros.
...
- Se quitó, por no encajar todavía con este flujo dirigido por datos: la pelea guionizada anterior (jugador vs. orco con stats hardcodeadas), el modo `simulate <N>` de simulación masiva, y la skill de ejemplo `Power Strike`. Se pueden recuperar/adaptar cuando volvamos a balance masivo o a habilidades.
