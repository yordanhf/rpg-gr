...
- **`CharacterSave`** (`src/World/CharacterSave.cs`) es un DTO plano, separado de `Combatant` (que lleva estado de ejecución: cooldowns, buffs, estado de combate). Guarda nombre, **raza por id**, **habitación por id (`roomId`)**, HP/MP/gold actuales y máximos, las 9 stats/skills. Lleva `Version` para poder migrar el formato. `CharacterSave.From(combatant, roomId)` y `ToCombatant(findRace, startingWeapon)` hacen la conversión. **No se guardan** cooldowns, buffs ni los emotes personalizados del arma.
